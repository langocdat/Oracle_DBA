...
SQL> ```SELECT owner, table_name, tablespace_name FROM dba_tables WHERE table_name = 'EMPLOYEE';```
    ```
...
SQL> ```SELECT owner, segment_name, tablespace_name, ROUND(SUM(bytes)/1024/1024, 2) AS size_mb FROM dba_segments WHERE segment_name = 'EMPLOYEE' GROUP BY owner, segment_name, tablespace_name;```
    ```
...
SQL> ```SELECT owner, segment_type, segment_name, tablespace_name, ROUND(SUM(bytes)/1024/1024, 2) AS size_mb FROM dba_segments WHERE segment_name = 'EMPLOYEE' OR segment_name IN (SELECT index_name FROM dba_indexes WHERE table_name = 'EMPLOYEE') GROUP BY owner, segment_type, segment_name, tablespace_name ORDER BY owner, segment_type;```
    ```
